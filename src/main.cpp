...
PWMDAC_CREATE_WAVETABLE(sawWavetable, PWMDAC_SAWTOOTH_WAVE);  // 矩形波
...
unsigned long kaiseki(char *data, unsigned char ch)
{
  ...
  while (play_flg != END)
  {
    ...
    case 'c': //ド
      ...
      noteOn(ch + 1, note_number[ch], 128);
      break;
    case 'd': //レ
...
void noteOn(uint8_t ch, uint8_t note, uint8_t vel)
{
  ...
  if (vel == 0)
  {
    PWMDACSynth::noteOff(ch, note, vel);
  }
  else
  {
    PWMDACSynth::noteOn(ch, note, 1);
  }
}